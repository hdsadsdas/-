...
func (cl *Cli)showPirkey()  {
	...
	prikey, err := bc.Wallet.ShowPrivateKey(*addr)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func (cl *Cli) checkaddr() {
	...
	send := flag.NewFlagSet("checkaddr", flag.ExitOnError)
	address := send.String("address", "", "需要验证的地址")
	...
	bo := bc.Wallet.Checkaddr(*address)
